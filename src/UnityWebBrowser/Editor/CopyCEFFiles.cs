...
public static class CopyCEFFiles
{
    private static readonly string[] fileTypes =
        {".pak", ".exe", ".dat", ".bin", "chrome-sandbox", "cefsimple", "CefBrowserProcess"};
    ...
    public static void OnPostprocessBuild(BuildTarget target, string pathToBuiltProject)
    {
        ...
        IEnumerable<string> files =
            Directory.EnumerateFiles(cefProcessDir, "*.*", SearchOption.AllDirectories).Where(file =>
                fileTypes.Any(x => file.EndsWith(x, StringComparison.OrdinalIgnoreCase)));
